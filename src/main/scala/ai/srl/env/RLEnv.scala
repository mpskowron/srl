...
//  def step(action: Ac): ZIO[Any, Throwable, EnvStep[Ac, Observation]]
  ...
  def foldZIO(
      initialState: State
  )(policy: Observation => ZIO[Any, Nothing, Ac], stateMapper: (Observation, State, Ac) => State): ZIO[Any, Nothing, Chunk[State]]
...
object RLEnv:
  ...
  def timeseriesRlEnvLayer[Ac: Tag, Observation: Tag, State: Tag, THS <: Int: ValueOf: Tag]: ZLayer[
    ...
    yield new RLEnv[Ac, SizedChunk[THS, Observation], State]:
      override def foldZIO(initialState: State)(
        policy: SizedChunk[THS, Observation] => ZIO[Any, Nothing, Ac],
        stateMapper: (SizedChunk[THS, Observation], State, Ac) => State
      ): ZIO[Any, Nothing, Chunk[State]] =
        ...
        for
          ...
          states = observations.zip(actions).mapAccum(initialState) { case (state, (observation, action)) =>
            val newState = stateMapper(observation, state, action)
            (newState, newState)
          }
        yield states._2
  }
  ...
  def foldZIO[Ac: Tag, Observation: Tag, State: Tag](
      initialState: State
  )(
      policy: Observation => ZIO[Any, Nothing, Ac],
      stateMapper: (Observation, State, Ac) => State
  ): ZIO[RLEnv[Ac, Observation, State], Nothing, Chunk[State]] =
