...
class EpsilonGreedyPolicy[Ac, Obs, P](val basePolicy: P, val exploreRate: Tracker)(using PurePolicy[P, Ac, Obs]):
...
object EpsilonGreedyPolicy:
  given [Ac, Obs, P : Description : HasName](using PurePolicy[P, Ac, Obs]) : Description[EpsilonGreedyPolicy[Ac, Obs, P]]
  with
  ...
  given [Ac, Obs, P : HasName](using PurePolicy[P, Ac, Obs]) : HasName[EpsilonGreedyPolicy[Ac, Obs, P]] with
    extension (policy: EpsilonGreedyPolicy[Ac, Obs, P]) 
      def name = s"${policy.getClass.getSimpleName}(${policy.basePolicy.name})"

  given [Ac, Obs, P: HasTrainer](using PurePolicy[P, Ac, Obs]): HasTrainer[EpsilonGreedyPolicy[Ac, Obs, P]] with
    extension (policy: EpsilonGreedyPolicy[Ac, Obs, P])
      def trainer = policy.basePolicy.trainer

  given [Ac, Obs, P](using PurePolicy[P, Ac, Obs]): PurePolicy[EpsilonGreedyPolicy[Ac, Obs, P], Ac, Obs] with
